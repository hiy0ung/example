...
interface ItemComponentProps {
  item: IItem; // id, name, amount (객체)

  onRemove: (id: number) => void; // 메서드(함수)
  onUpdate: (id: number, amount: number) => void; // 메서드(함수)
}
  ...
  return (
    <div>
      ...
      <input
        type="number"
        value={item.amount}
        onChange={(e) => onUpdate(item.id, Number(e.target.value))} // target - input 속성 (target.value - input 안의 value 속성)
      />
...
export default function UseRef03() {
  ...
  //? id값을 전달 받아 삭제하고자 하는 요소를 filtering 하는 함수
  ...
  return (
    <div>
      ...
      */}
      {/* 
        index는 계속 변하기 때문에 사용 권장 X
        {initialItems.map((item, index) => (
        // map과 filter 사용 시 생성되는 컴포넌트 도는 요소에는
