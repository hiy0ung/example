...
// - 간결한 API를 사용 (HTTP 요청과 응답 처리가 간결하고 직관적임)
// - 브라우저의 호환성이 높음
// - 자동으로 JSON 데이터 변환을 지원 (외부(jsonplaceholder 등)에서 데이터를 가져올 때.json() 사용하지 않아도 됨)
...
// 2) 'jsonplaceholder에서' 데이터 가져오기 / 전송하기
...
export default function Axios01() {
  ...
  const [users, setUsers] = useState<User[] | null>(null); // map 로직에 조건 추가 >> map은 배열에서 돌아야하니까 null 값이 들어갈 수 없음

  // 새로운 사용자에 대한 입력 데이터를 관리하는 상태 관리
  ...
  // async function fetchUsers() {} 화살표 함수로 표기
  const fetchUsers = async () => {
    try {
      ...
      // 외부 라이브러리는 전부 import 받아와야함 (axios 등)
      ...
      // setUsers(response.data) >> const data랑 합친 거
      setUsers(data);
    } catch (e) {
