...
class Book {
  ...
  // 메서드 정의
  rent() { // 책 대여
    if (this.isAvailable) { // 책이 대여 가능한 상태 (불리언 타입이라 === true 생략 가능)
...
class Library {
  ...
  // cf) 클래스의 메서드 정의 시 함수의 형태이지만 function 키워드 사용 X
  ...
  displayBooks() {
    ...
    this.books.forEach(book => {
      console.log(`${book.id}: ${book.title} by ${book.author} - ${book.isAvailable ? '대여 가능' : '대여됨'}`);
    })
  }
