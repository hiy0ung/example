...
function addToCart(id, name, price, quantity) {
  ...
  const index = cart.findIndex(item => item.id === id); // 객체의 id에 접근해서 매개변수 id와 같은지 

  // 상품이 이미 장바구니에 있을 경우
  if (index > -1) {
    cart[index].quantity += quantity; // 해당 상품의 수량을 업데이트
  } else { // 상품이 장바구니에 없을 경우
    cart.push({ id, name, price, quantity }); // 새 상품을 장바구니에 추가 (해당 변수 순서대로 cart배열에 추가)
  }
...
function updateQuantity(id, quantity) { // 상품을 구분할 수 있는 id 값, 수량을 변경할 수 있는 quantity 값 사용
...
//? 5. 장바구니의 총합을 계산하는 함수
// : reduce() 메서드
// 배열.reduce((acc, value, index, array) => { }, initialValue) / acc (축적)
