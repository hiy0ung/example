...
// 2. 할 일 수정: 완료된 할 일의 conpleted값을 전환(토글, toggle)
// >> 할 일의 완료 상태를 변경

// 3. 할 일 삭제: 원하는 할 일을 목록에서 제거

// 4. 할 일 목록을 출력: 현재 목록의 상태를 콘솔에 출력
...
function addTodo(content) { // 함수 호출 시 인자로 할 일의 내용을 전달받음
  ...
  const newTodo = {
    ...
    content: content, // 함수 호출 시 전달 받은 매개변수 값으로 지정
    completed: false
  }

  todos.push(newTodo); // 할 일 목록 마지막에 추가해줌
  displayTodos();
}
...
function toggleTodo(id) {
  todos = todos.map((todo) => { // map을 사용해 순회 하고 다시 전체 'todos'에 담아야함
    // 순회되는(todo) 할 일의 id와 매개변수로 전달 받은 id가 '일치한다면' 
    if (todo.id === id) {
...
function deleteTodo(id) {
  todos = todos.filter(todo => todo.id !== id); // id 값 뺴고 나머지 값을 새로운 배열로 데려감 (제거 된 것 같은 효과)
  displayTodos();
}
