...
// >> is, has와 같이 동사와 명사를 결합은 변수명을 주로 사용

// is: 명사 >> isStudent?: 학생이니?
// has: 동사 >> hasEatLunch?: 점심 먹었어?

let person = { //# 객체 리터럴 방식 {} 중괄호 사용 (직접 명시)
  // 속성(변수)
...
// 배열 메서드에서 쓰이는 콜백 함수의 인자 (value, index, array) >> 원하는 인자만 선택해서 쓸 수 있음
...
// >> JSON.stringfy(객체데이터) 객체데이터 -> JSON 데이터
// >> JSON.parse(JSON데이터) JSON 데이터 -> 객체데이터
...
personCopy.name = '이도경';
console.log(personCopy); // { name: '이도경', age: 50, isStudent: true }

console.log(person); // { name: '선하영', age: 50, isStudent: true } >> 서로 독립적인 개체

//& 2. 배열과 객체를 활용한 데이터 처리
// : 아래의 users 배열에서 성인 사용자(18세 이상)만 필터링(filter)하고, 필터링된 사용자의 이름을 새 배열로 만들어 반환하는(map) 함수(function~~)를 작성
// : 메서드 체이닝 사용
