...
//? cf) 배열(리스트)은 각 요소 접근 시 인덱스 변호를 사용
let array = [1, 2, 3, 4];
array[0] = 5; // 0번째 인덱스 값 5로 지정
...
let dog = {
  ...
  greet: function() {
    ...
    console.log(`hello, ${this.name.last}`); // this (나자신)에 있는 속성 name.last 접근 === dog.name.last와 동일 (해당 객체 안에서 !!!)
  }
}

// 1) 점 표기법 사용 (프로퍼티(속성) 접근)
...
// 만약 return 값 함수 호출 시, 앞에 let result = dog.greet() 등으로 담아서 하면 됨
...
// - 브라우저 환경에서는 window만약 return 값 함수 호출 시 앞에 let result = dog.greet() 등으로 담아서 하면 됨
...
// 생성자 함수를 사용한 객체 생성 시
// : this 값은 현재 쓰이는 객체에 바인딩 (어떠한 객체를 호출하냐에 따라 달라짐)
...
myObject.showThis(); //# this: myObject
*/
const arrowObject = {
  name: 'object',
  showThis: () => { //# 화살표 함수 사용
