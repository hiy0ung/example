...
document.addEventListener('DOMContentLoaded', () => {
  ...
  todos = JSON.parse(localStorage.getItem('todos')) || []; // null 값에 대한 조건식 활용
...
function addTodo() {
  ...
  if (newTodo) {
    ...
    renderTodos(todos);
    input.value = ''; // input 사용하고 나면 빈 문자열로 초기화
  }
}
...
function renderTodos(todos) {
  ...
  todos.forEach((todo, index) => {
    ...
    // deleteBtn.addEventListener('click', renderTodos(index)); // 53번줄이랑 같은 값
