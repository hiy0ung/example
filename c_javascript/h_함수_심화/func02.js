...
//* 배열 콜백 함수의 '인자값'
function callback (value, index, array) {} // value, index, arrya >> 인자값
...
// function (, index) {} // index만 사용할 경우 비우면 됨
// function (, , array) {} // array만 사용할 경우 비우면 됨

// : 매개변수는 함수 내부의 로컬 변수이기 때문에 이름 변경 가능

//# 배열의 콜백함수를 가지는 메서드 종류 >> 중요 !!
//& 1. forEach()
// : 각 요소에 대해 동일한 함수를 실행
...
numbers.forEach(function (value, index, array) { // 익명 함수? 사용
  // 모든 요소들에 동일한 기능 적용
  // : 단순한 기능 적용에 불과
...
const newNumbers = numbers.map(function(value) { // 익명 함수? 사용
  let sqaure = value * value;
  return sqaure;
});
// : 배열의 메서드로 반환되는 값은 자동으로 배열 내에 저장

// const arrowMap = basicArray.map((value) => {
//   return value * value;
// });
// 위의 식 축약
...
//# === 배열을 활용한 콜백 함수 예제 ===
let cars = ['audi', 'bmw', 'volvo', 'hyndai'];

// 1. 전체 배열의 요소를 순회하여 출력 - forEach
cars.forEach((car) => {
  console.log(car);
});
...
.map(value => value * value) // 위의 식이 배열이니까 바로 .기능동작 넣을 수 있음
