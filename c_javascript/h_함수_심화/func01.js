...
// 복잡한 로직 (예시)
function print(index) { // print는 콘솔창에 출력
  console.log(`${index}번째 함수 호출`);
} 

callback1(print); // print 함수(콜백함수)를 callback1(메인로직)의 인자로 전달
...
function callback2(number, callbackFunc, other) { // number 변수(데이터), callbackFunc 함수
  ...
  if (number % 2 === 0) { // 조건에 따른 콜백 함수를 호출
    callbackFunc(number);
  } else {
    other(number);
  }
...
// 메인 로직에 데이터(number)와 콜백함수(print3)를 전달
