...
fetchButton?.addEventListener('click', async () => {
  ...
  const apiUrl = `https://jsonplaceholder.typicode.com/users/${userId}`; // ``으로 JSONPlaceholder의 주소값 가져오기 /${userId} (input에서 입력 받은 값)

  if (userDataDiv) {
    // 해당 요소의 내부 HTML(innerHTML)로 로딩 메시지 출력
    // : Loading user data
    userDataDiv.innerHTML = `<p>Loading user data 속성</p>`;
