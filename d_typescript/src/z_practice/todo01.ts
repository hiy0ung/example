...
function addTodo(todos: ITodoItem[], task: string): ITodoItem[] {
  ...
  const newTodo: ITodoItem = {
    ...
    // Math.max(배열을 순회하여 가장 큰 id값을 가져옴) >> length 사용하면 삭제 기능 사용시 중복 충돌 날 수 있음
    id: Math.max(0, ...todos.map(todo => todo.id)) + 1, // [1, 2, 3, ...]
    task: task,
    completed: false,
  }

  // 기존 할 일 목록에 새로운 할 일 추가
  // : 스프레드 연산자 (기존의 리스트 요소 + 새로운 요소)
