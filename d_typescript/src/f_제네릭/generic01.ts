...
// "타입 변수"
// : 임의의 키워드를 사용하여 명시
// : 일반적으로 대문자 알파벳 하나를 사용 (T: type, U(T 다음으로 오는 알파벳, 의미 X), ...)
// >> 해당 타입 변수는 컴포넌트 내에서 실제 데이터 타입의 자리를 대신

function generic<T>(arg: T): T { // 함수 정의 시 타입 변수 지정 (보통 반환 타입은 사용 잘 안 함)
  // 현재 타입 변수 T (단일 타입 변수)
  return arg;
}
...
// let str3 = generic<string>(100); // Error - 'number' 형식의 인수는 'string' 형식의 매개 변수에 할당될 수 없습니다.
...
function pair2<T, U>(first: T, second: T): U { // U를 T[]로 작성 가능
  return [first, second] as U; // as U - U 타입 가질 거라고 타입 단언
}

let pair2Output = pair2<string, string[]>('hello', 'hi'); // ['hello', 'hi'] 
