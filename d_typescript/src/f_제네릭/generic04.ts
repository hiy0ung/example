...
function updateUser(id: number, changes: Partial<IUser>) {
  const user = users[id]; // IUser 타입의 객체
...
let user: Readonly<IHuman> = {
  // 값의 변경이 안 되는 옵션
  name: '이승아',
  age: 50
}

// user.name  = '서호영'; // Error - 읽기 전용 속성이므로 'name'에 할당할 수 없습니다
...
// interface IEmployeeNotId { - 문법에는 맞지만 불필요한 로직
//   name: string;
//   age: number;
//   position: string;
// }

type EmployeeWithoutID = Omit<IEmployee, 'id'>;
