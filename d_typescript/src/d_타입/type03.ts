...
//   return user.id != '';
// } // 함축 전
const isValidUser: ValidUser = (user: User) => user.id != ""; // 함축 후
...
// - 함수 표현식
// - 화살표 함수*
type FuncType = (num: number) => number; // type FuncType = (매개변수값 타입) => 리턴값 타입

// 함수의 타입 별칭을 사용하는 함수는 반드시 화살표 함수의 체계를 가져야 함
const exampleFun1: FuncType = (num) => {
  num *= 2; // num = num * 2;
  return num;
};

const exampleFun2 = (num: number): number[] => {
  num *= 2; // num = num * 2;
  return [num, num];
};
...
console.log(`${product.id} - ${product.name} / ${product.price}`); // abcde - 선하영 / 50000