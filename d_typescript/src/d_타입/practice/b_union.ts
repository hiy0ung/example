...
function handleResponse(response: FetchResponse) {
  ...
  // in 연산자
  // : '속성' in 객체명
  // >> 해당 속성이 객체 안에 있는지 여부를 boolean 타입으로 반환
...
async function fetchUserData(userId: number) {
  ...
  try {
    ...
    /*
    {
      error: 값 (삼항연산자 계산, error: e instanceof Error ? e.message : 'Unknown Error')
    }
    */

    // A instanceof B
    // : A 데이터가 B 안에 포함되어 있는지 여부를 확인 (상속되어 있는지 여부)
...
fetchUserData(1);
fetchUserData(2);
fetchUserData(222); // 에러관리가 가능한 오류
