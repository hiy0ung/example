...
// >> 해당 값을 자동으로 리터럴 값으로 유추 (리터럴 값 - 문자 그대로를 타입으로 봄)
...
greet(p1); // Person과 구조가 일치하기 때문에 Person 자체로 취급
...
type UserDataType = {
  ...
  // 키의 타입과 값의 타입을 미리 명시
  [key: string]: string | number | boolean; // 키는 string 사용을 권장 (key값은 ''를 넣지 않아도 기본적으로 string으로 인식하기 때문에)
...
// user1.email = [123, 456]; // 숫자배열은 지정한 타입에 없어서 불가
user1.email = "qwe123"; // 타입 형식만 맞추면 새로운 속성값도 생성 가능