{
  ...
  // 매개변수
  // - page: 현재 출력되는 페이지
  // - limit: 한 페이지 당 출력되는 사용자의 데이터 개수 (3으로 고정)

  // 반환 타입: Promise<Users>
  // >> async, await도 Promise를 기반으로 하기 때문에
  //    : Promise로 반환되는 데이터에 제네릭을 사용하여 원하고자 하는 데이터 형식으로 반환
  ...
  // 현재 페이지를 수를 기본값 1페이지로 저장
  let currentPage = 1;

  //# 현재 페이지 정보를 수정하는 함수
  const updatePageInfo = (): void => {
    // 페이지의 정보를 담는 DOM 요소 (span)
